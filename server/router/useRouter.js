    ...
    await bcrypt.compare(password, findone.password, (err, isValid) => {
      if (isValid) {
        ...
        res
          ...
          .status(200)
          .send({ message: "Logged in ", token, created: true });
      } else {
...
router.post(
  ...
  [
    ...
    check("address", "Enter address to ship your product").not().isEmpty(),
    check("password", "Min password length must be 5")
      .not()
      .isEmpty()
      .isLength({ min: 5 }),
    check("confirm_password", "Min password length must be 5")
      .not()
      .isEmpty()
      .isLength({ min: 5 }),
  ],
